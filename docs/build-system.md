...
`sha256` is optional for `http(s)://` URLs too. When given, resolving a `[native]` entry downloads it into a shared local cache at `~/.mira/packages/native/<sha256>/<basename>`, keyed purely by content hash: a cache hit is trusted without re-hashing, and there's nothing "mutable" to re-resolve the way a git branch or tag can move — the hash _is_ the pin. Omitting it still downloads and caches the jar (keyed by the URL itself instead), but a cache hit then only means "this URL was fetched before" — there's no verification that the content still matches what you expect. Prefer setting `sha256` when you know it; omit it when you don't want the chicken-and-egg problem of needing the file's hash before you can even declare the dependency. Only `http(s)://` and local `file://` URLs are supported today.
