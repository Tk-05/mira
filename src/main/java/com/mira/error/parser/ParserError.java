...
public class ParserError extends MiraError {
    ...
    public static class LexemeMismatchError extends ParserError {
        ...
        public LexemeMismatchError(Token token, String message) {
            super("E102", message + ", but found '" + token.getLexeme() + "'", token.getLine(), token.getColumn(),
                    spanOf(token),
        ...
        public LexemeMismatchError(Token found, Token after, String message) {
            super("E102", message + ", but found '" + found.getLexeme() + "'", after.getLine(), after.getColumn(),
                    spanOf(after), null);
        }
    ...
    public static class TypeMismatchError extends ParserError {
        ...
        public TypeMismatchError(Token token, String message) {
            super("E103", message + ", but got '" + token.getLexeme() + "'", token.getLine(), token.getColumn(),
                    spanOf(token), null);
        }
    }
    ...
    public static class UnexpectedToken extends ParserError {
        ...
        public UnexpectedToken(Token token, String message) {
            super("E101", message, token.getLine(), token.getColumn(), spanOf(token), null);
        }

        public UnexpectedToken(Token token, String message, String hint) {
            super("E101", message, token.getLine(), token.getColumn(), spanOf(token), hint);
        }
    }
}
