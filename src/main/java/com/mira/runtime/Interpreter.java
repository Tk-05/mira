...
public class Interpreter implements ExprVisitor<Object>, StmtVisitor<Object> {
    ...
    public <T> T visitCallExpr(CallExpression expression) {
        ...
        if (arguments.size() != callable.getArity()) {
            throw new ArgMismatchError((String) expression.getCallee().accept(this), callable.getArity(),
                    arguments.size());
        }
    ...
    public Object visitFor(For stmt) {
        ...
        while (loop) {
            for (Expression expression : stmt.getConditions()) {
                String condition = (String) expression.accept(this);
                if (!(boolean) Evaluator.evaluate(condition)) {
                    loop = false;
                }
            }
